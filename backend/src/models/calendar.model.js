...
function applyCalendarFilters(query, { search, year, month, branchId, academicYearId, eventType, status }) {
  if (search) {
    query.where((builder) => {
      builder
        .where('school_calendar.code', 'like', `%${search}%`)
        .orWhere('school_calendar.title', 'like', `%${search}%`);
    });
  }
...
const Calendar = {
  ...
  findAll(filters = {}) {
    const { search, year, month, branchId, academicYearId, eventType, status, page, pageSize } = filters;
    let query = db(TABLE)
    ...
    applyCalendarFilters(query, { search, year, month, branchId, academicYearId, eventType, status });
  ...
  async count(filters = {}) {
    const { search, year, month, branchId, academicYearId, eventType, status } = filters;
  ...
  findByMonth(year, month, branchId = null, academicYearId = null, page = null, pageSize = null) {
    let query = db(TABLE)
      .whereNull('school_calendar.deleted_at')
  ...
  async countByMonth(year, month, branchId = null, academicYearId = null) {
    let query = db(TABLE)
