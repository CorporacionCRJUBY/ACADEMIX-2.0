...
const create = async (req, res, next) => {
  try {
    const data = await creditsService.create(req.body, req.user);
...
const update = async (req, res, next) => {
  try {
    const data = await creditsService.update(req.params.id, req.body, req.user);
    res.json({ success: true, data });
  } catch (error) {
    next(error);
  }
...
const softDelete = async (req, res, next) => {
  try {
    await creditsService.softDelete(req.params.id, req.user);
...
const recalculate = async (req, res, next) => {
  try {
    const data = await creditsService.recalculate(req.params.studentId, req.user);
    res.json({ success: true, data });
  } catch (error) {
    next(error);
  }
