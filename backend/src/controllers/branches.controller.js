...
const create = async (req, res, next) => {
  try {
    const data = await branchesService.create(req.body, req.user);
...
const update = async (req, res, next) => {
  try {
    const data = await branchesService.update(req.params.id, req.body, req.user);
    res.json({ success: true, data });
  } catch (error) {
    next(error);
  }
...
const softDelete = async (req, res, next) => {
  try {
    await branchesService.softDelete(req.params.id, req.user);
