...
const create = async (req, res, next) => {
  try {
    const data = await permissionsService.create(req.body, req.user);
...
const update = async (req, res, next) => {
  try {
    const data = await permissionsService.update(req.params.id, req.body, req.user);
    res.json({ success: true, data });
  } catch (error) {
    next(error);
  }
...
const softDelete = async (req, res, next) => {
  try {
    await permissionsService.softDelete(req.params.id, req.user);
